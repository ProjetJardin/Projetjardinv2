...
// Couleurs reprises de la nouvelle identité visuelle (cf. css/style.css)
const VERT_FONCE = rgb(0.184, 0.290, 0.220);   // #2F4A38 (vert profond)
const VERT_MOUSSE = rgb(0.306, 0.478, 0.337);  // #4E7A56 (vert signal)
...
function dessinerDecor(page) {
  ...
  page.drawEllipse({ x: -60, y: 20, xScale: 260, yScale: 110, color: VERT_MOUSSE, opacity: 0.9 });
...
function dessinerLogoEtTitre(page, { logoImage, doc, police, policeGrasse }) {
  ...
  page.drawText(titre, {
    ...
    size: taille, font: policeGrasse, color: VERT_FONCE,
  });
  page.drawLine({
...
function dessinerTableau(page, contexte, yDepart, creerNouvellePage) {
  ...
  function dessinerEntete() {
    page.drawRectangle({ x: MARGE, y: y - hauteurEntete, width: largeurTableau, height: hauteurEntete, color: VERT_MOUSSE });
    let x = MARGE + 10;
  ...
  prestations.forEach((ligne, index) => {
    ...
    if (index % 2 === 0) {
      page.drawRectangle({ x: MARGE, y: y - hauteurLigne, width: largeurTableau, height: hauteurLigne, color: GRIS_CLAIR });
    }
...
function dessinerTotaux(page, { doc, policeGrasse, police }, yDepart) {
  ...
  const largeurPastille = policeGrasse.widthOfTextAtSize(texteTtc, 11) + 24;
  page.drawRectangle({
    x: LARGEUR_PAGE - MARGE - largeurPastille, y: y - 6, width: largeurPastille, height: 24,
    color: VERT_MOUSSE,
  });
  page.drawText(texteTtc, {
    x: LARGEUR_PAGE - MARGE - largeurPastille + 12, y: y, size: 11, font: policeGrasse, color: BLANC,
  });
...
function dessinerPiedDePage(page, { doc, parametres, police, policeGrasse }, yDepart) {
  ...
  if (doc.mode_paiement) {
    page.drawText('Mode de règlement :', { x: MARGE, y, size: 10, font: policeGrasse, color: VERT_FONCE });
    y -= 15;
  ...
  if (doc.notes) {
    page.drawText('Notes :', { x: MARGE, y, size: 9, font: policeGrasse, color: VERT_FONCE });
    y -= 13;
