...
[champRecherche, filtreType, filtreAnnee, filtreStatut].forEach(el => el.addEventListener('input', appliquerFiltres));
          ...
          ${docs.map(d => `
            <tr data-id="${d.id}" data-type="${d.type}" data-statut="${d.statut}">
              ...
              <td class="actions-ligne">
                ...
                ${d.statut === 'brouillon' ? `<button class="bouton bouton-danger bouton-petit" data-action="supprimer-brouillon">Supprimer</button>` : ''}
              </td>
            </tr>
          `).join('')}
        </tbody>
...
zoneTableau.addEventListener('click', async (e) => {
  ...
  try {
    ...
    if (bouton.dataset.action === 'supprimer-brouillon') {
      await supprimerBrouillon(id, ligne.dataset.type, ligne.dataset.statut);
    }
  } catch (err) {
    console.error(err);
...
// La suppression n'est jamais possible pour un statut autre que "brouillon" :
// dès qu'un document a été envoyé, accepté, facturé ou payé, il doit rester
// dans l'historique.
async function supprimerBrouillon(id, type, statut) {
  if (statut !== 'brouillon') {
    afficherToast("Seuls les documents à l'état « brouillon » peuvent être supprimés.", 'erreur');
    return;
  }
  const ok = await confirmer(
    `Supprimer ce ${type === 'devis' ? 'devis' : 'cette facture'} ?`,
    'Ce document est encore un brouillon : il sera définitivement supprimé, ainsi que ses prestations.',
    'Supprimer'
  );
  if (!ok) return;

  const { error } = await supabase.from('documents').delete().eq('id', id).eq('statut', 'brouillon');
