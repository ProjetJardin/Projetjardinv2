...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // Sauvegarde des prestations ligne par ligne (mise à jour si la ligne
    // existait déjà, création sinon), plutôt que de tout supprimer puis
    // tout réinsérer : ça évite de perdre des données si une étape échoue
    // en cours de route.
    const idsConserves = new Set(lignesPrestations.filter(l => l.id).map(l => l.id));
    const idsASupprimer = idsPrestationsChargees.filter((id) => !idsConserves.has(id));

    if (idsASupprimer.length > 0) {
      const { error: erreurSuppression } = await supabase.from('prestations').delete().in('id', idsASupprimer);
      if (erreurSuppression) throw erreurSuppression;
    }

    for (const ligne of lignesPrestations) {
      const { id, ...donneesLigne } = ligne;
      if (id) {
        const { error: erreurMaj } = await supabase.from('prestations').update(donneesLigne).eq('id', id);
        if (erreurMaj) throw erreurMaj;
      } else {
        const { error: erreurCreation } = await supabase.from('prestations').insert({ ...donneesLigne, document_id: idFinal });
