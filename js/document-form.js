...
const retourVers = parametresUrl.get('retour') === 'dashboard' ? 'dashboard' : 'documents';

// Garde-fou contre le cache de navigation (bfcache) sur mobile : si la page
// est restaurée depuis ce cache (ex: bouton "précédent" de Safari) plutôt
// que rechargée, on force un vrai rechargement pour être certain de
// toujours repartir des données à jour de Supabase.
...
async function chargerDocumentExistant(id) {
  try {
    ...
    idsPrestationsChargees = (prestations || []).map((p) => p.id);
...
function ajouterLignePrestation(donnees = null) {
  ...
  ligne.dataset.prestationId = donnees?.id || '';
  if (donnees) {
...
form.addEventListener('submit', async (e) => {
  ...
  const lignesPrestations = [...corpsPrestations.querySelectorAll('tr')].map((ligne, index) => {
    ...
    if (afficherTva) totalTva += totalLigne * (tva / 100);
    return {
      id: ligne.dataset.prestationId || null,
  ...
  try {
    ...
    // Toutes les lignes actuellement affichées sont insérées comme de
    // NOUVELLES lignes en base — jamais de mise à jour "en place". Ce n'est
    // qu'une fois cette insertion confirmée réussie qu'on supprime les
    // anciennes lignes. De cette façon, si quoi que ce soit échoue en cours
    // de route, on ne perd jamais de données existantes (au pire, on garde
    // les anciennes lignes en double plutôt que de tout perdre).
    if (lignesPrestations.length > 0) {
      const { error: erreurCreation } = await supabase.from('prestations').insert(
        lignesPrestations.map(({ id, ...donneesLigne }) => ({ ...donneesLigne, document_id: idFinal }))
      );
      if (erreurCreation) throw erreurCreation;
    }

    if (idsPrestationsChargees.length > 0) {
      const { error: erreurSuppression } = await supabase.from('prestations').delete().in('id', idsPrestationsChargees);
      if (erreurSuppression) throw erreurSuppression;
    }

    afficherToast('Document enregistré.');
    window.location.href = retourVers === 'dashboard' ? 'index.html' : 'documents.html';
  } catch (err) {
    console.error(err);
